...
- **Пошук шляху до ворожих ресурсів у граф-панелі**: при кліку на вузол графа
  в інфо-панелі з'являється посилання **🔍 Search for primary enemy resource**.
  По кліку виконується ненаправлений BFS від поточного вузла до всіх досяжних
  вузлів `PrimaryEnemyResources`; для кожного знайденого ресурсу показується
  найкоротший шлях із кількістю переходів. Клік по шляху підсвічує всі вузли
  та ребра на ньому і масштабує вигляд. Якщо шляхів немає — виводиться відповідне
  повідомлення. Граф відмальовується як завжди, функція не впливає на розмір файлу
  помітно (~2 KB JavaScript).

### Виправлено / Покращено

- `GraphExporter.buildJson()` тепер включає масив `primaryEnemyIds` у вбудований JSON
  графа, щоб JS міг ідентифікувати цільові вузли для BFS без жорсткого кодування.
