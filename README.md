...
Починаючи з версії 3.0.0 доступний повноцінний **графічний інтерфейс** (`-g` / `--gui`) з живим відображенням процесу обробки. Починаючи з версії 3.3.1 доступний **пакетний режим** (`-b` / `--batch`) — автоматичний запуск зовнішнього скрипту після завершення обробки.
...
```
target/ASBlockWar-3.3.1-<buildNumber>.jar
```

Запуск потребує встановленої JRE 25+ на цільовій машині:

```bash
java -jar target/ASBlockWar-3.3.1-00000001.jar [параметри]
```
...
```bash
java -jar ASBlockWar-3.3.1-00000001.jar --config=/etc/asblockwar/asblockwar.properties
```
...
```bash
java -jar target/ASBlockWar-3.3.1-00000001.jar --gui
```
...
- **Ліва колонка — акордеон із трьома вкладками:**
...
```bash
java -jar target/ASBlockWar-3.3.1-00000001.jar --batch
```
...
Повний ланцюг після одного запуску `java -jar ASBlockWar-3.3.1-00000001.jar --batch`:
