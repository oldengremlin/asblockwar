...
```
target/ASBlockWar-3.13.0-<buildNumber>.jar
```

Запуск потребує встановленої JRE 25+ на цільовій машині:

```bash
java -jar target/ASBlockWar-3.13.0-00000001.jar [параметри]
```
...
Модульні тести на JUnit 5. Наразі покривають `AsnRegexBuilder` — trie-оптимізатор
as-path regex: префіксні пари ASN, ланцюжки вкладених префіксів і брутфорс
на випадкових префіксно-насичених наборах з перевіркою, що згенерований regex
матчить **рівно** заданий набір ASN і жодного стороннього.
...
```bash
java -jar ASBlockWar-3.13.0-00000001.jar --config=/etc/asblockwar/asblockwar.properties
```
...
```bash
java -jar target/ASBlockWar-3.13.0-00000001.jar --gui
```
...
java -jar ASBlockWar-3.13.0-00000001.jar --dependency-graph

# Задати власний шлях
java -jar ASBlockWar-3.13.0-00000001.jar -dg /tmp/asblockwar-graph.html
```
...
java -jar ASBlockWar-3.13.0-00000001.jar --send-report \
...
```bash
java -jar target/ASBlockWar-3.13.0-00000001.jar --batch
```
...
Повний ланцюг після одного запуску `java -jar ASBlockWar-3.13.0-00000001.jar --batch`:
