...
Модульні тести на JUnit 5:

- `AsnRegexBuilderTest` — trie-оптимізатор as-path regex: префіксні пари ASN,
  ланцюжки вкладених префіксів і брутфорс на випадкових префіксно-насичених
  наборах з перевіркою, що згенерований regex матчить **рівно** заданий набір
  ASN і жодного стороннього.
- `PrefixValidationTest` — валідація `ForceNetBlock` (значення потрапляє прямо
  в команду роутера) і захист `STORE/` від виходу за межі директорії.
- `PrefixUtilsTest` — канонізація префіксів: різні записи однієї IPv6-мережі
  мають вважатися тим самим маршрутом, інакше він знімається й ставиться назад
  на кожному прогоні.
- `RpslUtilsTest` — розбір багаторядкових RPSL-полів і `mp-members`.
- `NetworkUtilsTest` — упорядкування CIDR-префіксів: IPv6 мають сортуватися
  числово, а не лексикографічно (перевірка проти еталона на `InetAddress`).
- `GraphBuilderTest` — розбір `members:` з continuation-рядками RFC 2622.
...
Булеві властивості (`BlackbgpIpv6`, `BatchMode`, `UseSfdp`, `DependencyWithUnknown`,
`SendEmailReport`) приймають `true/yes/on/1` і `false/no/off/0` у будь-якому регістрі.
На нерозпізнаному значенні пишеться попередження і береться значення за замовчуванням —
раніше все, крім рівно `true`, мовчки давало `false`.
...
з `0`, тож для cron чи systemd навіть повністю провалений прогін виглядав успішним.
...
- **Клік по вузлу** — відкриває панель деталей (id, тип, статус, country, org-name тощо),
  «тьмяніє» не пов'язані вузли, а ребра до безпосередніх сусідів підсвічуються яскраво-червоним
  (`#FF1744`, 3.5px) і піднімаються поверх решти ребер.
...
- **Фільтр за типом вузла** — чекбокси ASN / AS-SET / MNTNER / ORG приховують або показують відповідні типи вузлів (незалежно від фільтра за статусом). Кнопка Reset відновлює обидва фільтри.
- **Кнопка Reset** — знімає фокус і відновлює початковий вигляд (статус- і тип-фільтри, zoom).
...
з підсумковим звітом. Звіт містить п'ять секцій:

- **Зміни ASN** — таблиця доданих, вилучених і змінених ASN з country і org-name; для `modify`
  відображається **"було → стало"** з кольоровим кодуванням (темно-червоний → темно-зелений)
  якщо поля змінились між запусками; `add` — зелений, `remove` — червоний. Для `remove`
  і country, і org-name позначаються як **`було «Значення»`** — показане значення
...
- **Замасковані маршрути** — маршрути, зняття яких скасовано Перевіркою 3
  ...
  і країна origin-ASN; заблокована виділяється кольором. ASN і назва організації
  беруться з того, що прочитала сама перевірка: origin такого маршруту за
  побудовою не ворожий, тож ні в `aggressorAsnResources`, ні в кеші `STORE/AS/`
  його немає, і колонка «Організація» інакше лишалася б порожньою саме там, де
  вона найпотрібніша. Секція показується лише за наявності таких маршрутів;
- **Маршрути вилучені з blackbgp** (`ip r d`) — prefix / origin ASN / country / descr.
  ...
  до БД — для вже знятого маршруту мережа могла бути перепризначена стороннику,
  який ніколи не блокувався. Country й org-name завжди позначаються як
  **`було «Значення»`** — увесь рядок описує стан ДО зняття маршруту; якщо значення
...
| `--email-smtp-ssl-trust=<host\|*>` | `EmailSmtpSslTrust=` | Хост для SSL-довіри при STARTTLS/SSL; `*` = відключити перевірку (небезпечно) |
...
|---|---|---|
| `STORE/AS/` | `-ran {as}` | aut-num + org (для замаскованих — плюс покривний inetnum, див. нижче) |
...
`STORE/AS/` пишеться і для origin **замаскованих** маршрутів — попри те, що такий
ASN не входить до `list.txt`. Без цього незрозуміло, за що AS заблоковано: у самому
`aut-num` ворожої країни немає, вона є лише в покривному `inetnum`. Тому в кінець
файлу додається секція з тими об'єктами, з яких країну взято, — рядки пояснення
починаються з `%`, як коментарі whois, тож розбір RPSL їх не зачіпає:
...
org:            ORG-NO63-RIPE
...
organisation:   ORG-NO63-RIPE
org-name:       NetCrafters OU
country:        EE
source:         RIPE
...
Якщо один ASN тримає кілька замаскованих маршрутів, кожен іде окремою секцією зі
своїм `inetnum`. Для маршрутів, у яких `route:` прибрано (сценарій 2), origin
невідомий — писати нема для кого, і файл не створюється. Якщо ASN усе ж потрапив
до `list.txt` як ворожий, його пише звичайний цикл — двох записувачів в один шлях
не буває.
...
Якщо ASN зі списку є **префіксом** іншого ASN зі списку, хвіст стає необов'язковим
через `?`: `{2148, 21483}` → `2148[3]?`, `{219, 21907}` → `219(07)?`. Дужки навколо
багатосимвольного хвоста обов'язкові — без них `?` прив'язався б лише до останньої
цифри, і коротший ASN перестав би блокуватися.
...
Перед видаленням кожен маршрут-кандидат проходить три перевірки:
...
3. Чи не належить покривний `inetnum`/`inet6num` блокованій країні, попри
   «чистий» або взагалі відсутній `route:`? Див. нижче.

Перші дві перевірки запобігають випадковому розблокуванню маршруту ворожого ASN
лише тому, що він тимчасово випав із проміжного стану обробки. Третя — навмисному
обходу country-фільтра.

#### Замасковані маршрути (Перевірка 3)

RPSL **не несе `country:` на самому `route:`** — країна маршруту резолвиться
виключно через `origin:` → `aut-num`/`organisation`. Звідси два різні обходи
блокування, і жоден із них не потребує помилки в нашому коді.

##### Сценарій 1: маршрут переоформлено під ASN «чистої» країни

Мережу переоформлюють під ASN легітимного хостера з незаблокованої країни
(створюють точніший `route:` з його `origin:`), а фактичний власник лишається
в окремому ланцюжку `inetnum:` → `org:` → `organisation:`, якого перевірка за
origin ніколи не торкається. Формально за RPSL усе чесно — а фільтр за країною
пробито наскрізь.

Реальний випадок, `5.231.231.0/24`:

| Об'єкт | Поле | Значення |
|---|---|---|
| `route: 5.231.231.0/24` (створено 30.08.2026) | `origin:` | `AS12586` → GHOSTnet GmbH, `country: DE` |
| `inetnum: 5.231.231.0 - 5.231.231.255` | `country:` | `FI` |
| `organisation: ORG-MN228-RIPE` (з `org:` того inetnum) | `country:` | `RU`, адреса в Самарі, телефон `+7` |
...
##### Сценарій 2: `route:` прибрано зовсім

Простіший і дієвіший за перший. Об'єкт `route:` просто видаляють з RIPE —
і префікс тихо випадає з цільового набору, бо той будується через `origin:`.
Ні route → ні origin → ні країни → ні підстав тримати маршрут у blackbgp.
Тут узагалі ніде не з'являється жодного `country:`, тож перевіркам 1 і 2
нема за що зачепитися.

Реальний випадок, `45.82.152.0/24` — маршрут, який до 2026-09-03 успішно
знімався з blackbgp:

| Об'єкт | Поле | Значення |
|---|---|---|
| `route: 45.82.152.0/24` | — | **об'єкта немає** |
| `inetnum: 45.82.152.0 - 45.82.152.255` | `netname:` / `country:` | `RU-RUVDS-20211209` / `RU` (змінено 2026-09-01) |
| `role: RVS248-RIPE` (з `tech-c:` того inetnum) | `address:` / `phone:` | Москва / `+7 495 1351099` |

Саме тому Перевірка 3 **не** вимагає наявності `route:`. Умова «спершу переконайся,
що маршрут існує і його origin у чистій країні» виглядає природною, але вона б
відкрила цей сценарій навстіж.

##### Спільна перевірка

Для кожного маршруту, що йде на зняття, шукається **покривний**
...
За сценарієм 2 ланцюг вироджується в один запис — **`RU`**: origin-ASN немає,
країну з нього взяти нема звідки. Це не ознака збою: інших країн у таких даних
справді немає.

Механізм самокоригується. Якщо мережу колись передадуть власнику з незаблокованої
країни, `inetnum` це відобразить — і маршрут зніметься сам, без ручного втручання.
...
Якщо покривних об'єктів немає — поведінка не змінюється: рішення приймається
виключно за `route`/`route6`, як до появи Перевірки 3.
...
> «чистим» ASN, вона не знайде — для цього потрібен зворотний прохід
...
Зміни (вилучено / додано / модифіковано) виводяться у вигляді кількох таблиць в `INFO`.

Зведена таблиця (кількість та перелік за колонками):
...
Деталі змін — ASN, дія, країна та назва організації:

```
Деталі змін:
ASN    │ Дія      │ CN │ Організація
━━━━━━━┿━━━━━━━━━━┿━━━━┿━━━━━━━━━━━
AS1234 │ Вилучено │ RU │ Ворожий ISP LLC
AS5678 │ Додано   │ RU │ Агресор ТОВ
AS9012 │ Змінено  │ RU │ Оновлений ASP
```

Якщо є підозрілі AS поза BlockCountry — окрема таблиця після змін:

```
Підозрілі AS поза BlockCountry — збіг з AggressorPattern (1):
ASN    │ Країна │ Збіг з AggressorPattern 
━━━━━━━┿━━━━━━━━┿━━━━━━━━━━━━━━━━━━━━━━━━━
AS9999 │ CZ     │ org-name: Qrator Labs CZ
```
