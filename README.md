...
```
target/ASBlockWar-3.7.7-<buildNumber>.jar
```

Запуск потребує встановленої JRE 25+ на цільовій машині:

```bash
java -jar target/ASBlockWar-3.7.7-00000001.jar [параметри]
```
...
```bash
java -jar ASBlockWar-3.7.7-00000001.jar --config=/etc/asblockwar/asblockwar.properties
```
...
```bash
java -jar target/ASBlockWar-3.7.7-00000001.jar --gui
```
...
java -jar ASBlockWar-3.7.7-00000001.jar --dependency-graph

# Задати власний шлях
java -jar ASBlockWar-3.7.7-00000001.jar -dg /tmp/asblockwar-graph.html
```
...
```bash
java -jar target/ASBlockWar-3.7.7-00000001.jar --batch
```
...
Повний ланцюг після одного запуску `java -jar ASBlockWar-3.7.7-00000001.jar --batch`:
