...
```
target/ASBlockWar-3.15.0-<buildNumber>.jar
```

Запуск потребує встановленої JRE 25+ на цільовій машині:

```bash
java -jar target/ASBlockWar-3.15.0-00000001.jar [параметри]
```
...
```bash
java -jar ASBlockWar-3.15.0-00000001.jar --config=/etc/asblockwar/asblockwar.properties
```
...
```bash
java -jar target/ASBlockWar-3.15.0-00000001.jar --gui
```
...
java -jar ASBlockWar-3.15.0-00000001.jar --dependency-graph

# Задати власний шлях
java -jar ASBlockWar-3.15.0-00000001.jar -dg /tmp/asblockwar-graph.html
```
...
- **Зміни ASN** — таблиця доданих, вилучених і змінених ASN з country і org-name; для `modify`
  ...
  якщо поля змінились між запусками; `add` — зелений, `remove` — червоний;
...
java -jar ASBlockWar-3.15.0-00000001.jar --send-report \
...
```bash
java -jar target/ASBlockWar-3.15.0-00000001.jar --batch
```
...
Повний ланцюг після одного запуску `java -jar ASBlockWar-3.15.0-00000001.jar --batch`:
