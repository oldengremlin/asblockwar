...
Починаючи з версії 3.0.0 доступний повноцінний **графічний інтерфейс** (`-g` / `--gui`) з живим відображенням процесу обробки. Починаючи з версії 3.3.0 доступний **пакетний режим** (`-b` / `--batch`) — автоматичний запуск зовнішнього скрипту після завершення обробки. Версія 3.3.9 додає обов'язковий **country-фільтр** (`BlockCountry`, за замовчуванням `RU`) як першу умову блокування AS, вкладку **List Prefixes** з активними blackhole-маршрутами та підтримку IPv6 у blackbgp за замовчуванням.
...
```
target/ASBlockWar-3.3.9-<buildNumber>.jar
```

Запуск потребує встановленої JRE 25+ на цільовій машині:

```bash
java -jar target/ASBlockWar-3.3.9-00000001.jar [параметри]
```
...
```bash
java -jar ASBlockWar-3.3.9-00000001.jar --config=/etc/asblockwar/asblockwar.properties
```
...
Відкривається при натисканні *Run*. Відображає живий лог виконання (рівні INFO та вище) у текстовій прокручуваній панелі та невизначений індикатор прогресу. Кнопка *Close* стає активною тільки після завершення або помилки. Закрити вікно хрестиком під час виконання неможливо.

У пакетному режимі (`-b` / `--batch`) після завершення обробки у тій само панелі відображається вивід `AfterCommand`-скрипту: рядки зі stderr виводяться червоним кольором (`#cc0000`), рядки зі stdout — стандартним текстом.
...
| After command | файл |
| Block countries (CSV) | рядок, коди країн через кому (напр. `RU,BY`) |
...
flowchart TD
    ...
    MR --> F2["[5] filterAggressorAsnResources ②\nфінальна фільтрація (country + AGGRESSOR_PATTERN)"]
    ...
    class MR,DC process
    class SM,WR,BG,WR2,ST,SD,AL,ML,NF,RP,BC output
```
...
Кроки `[1]` і `[2]` виконуються послідовно. Кроки `[8a]`/`[8b]` та `[11a]`–`[11d]` виконуються паралельно (virtual threads). Крок `[13]` виконується тільки у пакетному режимі (`-b` / `--batch`).
...
2. Чи належить він AS, яка виявилась ворожою саме зараз (нова, ще не в списку)?
   Перевірка виконується через той самий combined-фільтр (country + AGGRESSOR_PATTERN).
...
```bash
java -jar target/ASBlockWar-3.3.9-00000001.jar --batch
```
...
Повний ланцюг після одного запуску `java -jar ASBlockWar-3.3.9-00000001.jar --batch`:
