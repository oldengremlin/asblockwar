...
```
target/ASBlockWar-3.3.2-<buildNumber>.jar
```

Запуск потребує встановленої JRE 25+ на цільовій машині:

```bash
java -jar target/ASBlockWar-3.3.2-00000001.jar [параметри]
```
...
```bash
java -jar ASBlockWar-3.3.2-00000001.jar --config=/etc/asblockwar/asblockwar.properties
```
...
```bash
java -jar target/ASBlockWar-3.3.2-00000001.jar --gui
```
...
```bash
java -jar target/ASBlockWar-3.3.2-00000001.jar --batch
```
...
Повний ланцюг після одного запуску `java -jar ASBlockWar-3.3.2-00000001.jar --batch`:
