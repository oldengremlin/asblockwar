...
Починаючи з версії 3.0.0 доступний повноцінний **графічний інтерфейс** (`-g` / `--gui`) з живим відображенням процесу обробки. Починаючи з версії 3.3.0 доступний **пакетний режим** (`-b` / `--batch`) — автоматичний запуск зовнішнього скрипту після завершення обробки. Версія 3.3.9 додає обов'язковий **country-фільтр** (`BlockCountry`, за замовчуванням `RU`) як першу умову блокування AS, вкладку **List Prefixes** з активними blackhole-маршрутами та підтримку IPv6 у blackbgp за замовчуванням. Версія 3.3.10 додає **примусове блокування** (`ForceASBlock`, `ForceNETBlock`) для AS та мереж, що обходять country/pattern-фільтри, а також перетворює `BlockCountry`, `ForceASBlock` і `ForceNETBlock` на редаговані списки у діалозі Properties. Версія 3.3.12 додає підтримку телефонних номерів `+7` у патерні агресора та виносить `AggressorPattern` у Properties (з валідацією regex). Версія 3.3.16 переводить вікно Run Progress на **WebView**: лог тепер доступний для виділення мишею та копіювання (Ctrl+C). Версія 3.3.17 замінює ручний парсер CLI-аргументів на **Picocli** (`--help` генерується автоматично з анотацій).
