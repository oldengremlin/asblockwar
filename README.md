...
3. Чи не замасковано маршрут під ASN «чистої» країни? Див. нижче.
...
виключно через `origin:` → `aut-num`/`organisation`. Це дає обхід блокування,
який не потребує жодної помилки в коді: мережу переоформлюють під ASN легітимного
...
не торкається. Формально за RPSL усе чесно — а фільтр за країною пробито наскрізь.
