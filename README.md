...
```
target/ASBlockWar-3.16.0-<buildNumber>.jar
```

Запуск потребує встановленої JRE 25+ на цільовій машині:

```bash
java -jar target/ASBlockWar-3.16.0-00000001.jar [параметри]
```
...
```bash
java -jar ASBlockWar-3.16.0-00000001.jar --config=/etc/asblockwar/asblockwar.properties
```
...
```bash
java -jar target/ASBlockWar-3.16.0-00000001.jar --gui
```
...
java -jar ASBlockWar-3.16.0-00000001.jar --dependency-graph

# Задати власний шлях
java -jar ASBlockWar-3.16.0-00000001.jar -dg /tmp/asblockwar-graph.html
```
...
- **Зміни ASN** — таблиця доданих, вилучених і змінених ASN з country і org-name; для `modify`
  ...
  org-name позначається як **`було «Назва»`** — показане значення не поточне, а те,
  яким воно було до видалення ASN;
- **Підозрілі AS поза BlockCountry** — AS, що збіглись з AggressorPattern, але не підпадають під блокування за країною;
- **Маршрути вилучені з blackbgp** (`ip r d`) — prefix / origin ASN / country / descr. Якщо org-name
  узято з кешу `STORE/AS/` (ASN, якого поточний запуск не торкався — можливо, об'єкт уже
  видалено з RPSL), назва так само позначається як **`було «Назва»`**; якщо назви немає
  навіть у кеші, мітка не виводиться;
...
java -jar ASBlockWar-3.16.0-00000001.jar --send-report \
...
```bash
java -jar target/ASBlockWar-3.16.0-00000001.jar --batch
```
...
Повний ланцюг після одного запуску `java -jar ASBlockWar-3.16.0-00000001.jar --batch`:
