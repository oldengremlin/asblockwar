...
- **Вихідний**: AS-SET-и, виявлені автоматично через `import`/`export`-поля ворожих ASN, дописуються при кожному запуску. Файл перезаписується відсортованим.

Вбудовані AS-SET-и (`AS-MAILRU`, `AS-VK`, `AS-VKONTAKTE`, `AS-YANDEX`, `AS-M100`) завжди обробляються незалежно від вмісту файлу.
